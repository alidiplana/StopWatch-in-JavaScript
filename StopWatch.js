class StopWatch{
    constructor(nanoSeconds = 0, miliSeconds = 0, seconds = 0, minutes = 0, hours = 0, status){
...
function stopWatch(){
    ...
    document.getElementById("display").innerHTML =  (sw.hours<10 ? "0" + sw.hours : sw.hours) + ":" + (sw.minutes<10 ? "0" + sw.minutes : sw.minutes) + ":" + (sw.seconds<10 ? "0" + sw.seconds : sw.seconds) +":" + sw.miliSeconds;
    document.getElementById("nano").innerHTML = + (sw.nanoSeconds< 10 ? "0" + sw.nanoSeconds : sw.nanoSeconds);
}
...
function splitWatch(){
    ...
    if(splits.length == 0){
        ...
        if(sw.miliSeconds - splits[splits.length-1].miliSeconds < 0){
            tempMiliSeconds = 10 - (sw.miliSeconds - splits[splits.length-1].miliSeconds);
            tempSeconds++;
        }
        else{
        ...
        if(sw.nanoSeconds - splits[splits.length-1].nanoSeconds < 0){
            tempNanoSeconds = 100 - ( sw.nanoSeconds - splits[splits.length-1].nanoSeconds);
            tempMiliSeconds++;
        }
        else{
        ...
        let newSW = new StopWatch(tempNanoSeconds, tempMiliSeconds, tempSeconds, tempMinutes , tempHours, status);
        splits.push(newSW);
        console.log("Hours: "+tempHours);
        console.log("Minutes: "+tempMinutes);
        console.log("Seconds: "+ tempSeconds);
    ...
    for (var i = 0; i < splits.length; i++){
        var row = `<tr>
                        <td>${i+1}</td>
                        <td>${(splits[i].hours<10 ? "0" + splits[i].hours : splits[i].hours) + ":" + (splits[i].minutes<10 ? "0" + splits[i].minutes : splits[i].minutes) + ":" + (splits[i].seconds<10 ? "0" + splits[i].seconds : splits[i].seconds) +":" + splits[i].miliSeconds  + (splits[i].nanoSeconds< 10 ? "0" + splits[i].nanoSeconds : splits[i].nanoSeconds)}</td>
                        <td>${splits[i].status == true ? "Split" : "Paused"}</td>
                  </tr>`;
        // console.log(splits[i].minutes);
        table.innerHTML += row;
    }
